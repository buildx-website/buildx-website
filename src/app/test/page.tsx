...
function App() {
  ...
  return (
    <div>
      <button onClick={handleParseExample}>Parse Example XML</button>

      <div>
        <h2>Parsed Steps ({steps.length})</h2>
        <ul>
          {steps.map(step => (
            <li key={step.id}>
              <strong>{step.title}</strong> - {step.description}
              <div>Status: {step.status}</div>
              {step.path && <div>Path: {step.path}</div>}
            </li>
          ))}
        </ul>
      </div>
